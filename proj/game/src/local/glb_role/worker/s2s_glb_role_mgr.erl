...
%% ===================================================================================
ets_init()->
  gs_glb_role_mgr:ets_init(),
  ?OK.
start_sup_link()->
  gs_glb_role_mgr:start_sup_link(),
  ?OK.
...
new_child(GenId)when is_integer(GenId)->
  Pid = gs_glb_role_mgr:new_child({GenId}),
  Pid.
...
priv_stop(GenId) when is_integer(GenId)->
  gs_glb_role_mgr:stop(GenId),
  ?OK.

new_role(GlbRolePojo)->
  RoleId = glb_role_pdb_pojo:get_id(GlbRolePojo),
  GenId = RoleId rem ?MaxGenCount,
  priv_cast_fun(GenId,{fun bs_glb_role_mgr:new_role/1,[{GlbRolePojo}]}),
  ?OK.
update_role(GlbRolePojo)->
  RoleId = glb_role_pdb_pojo:get_id(GlbRolePojo),
  GenId = RoleId rem ?MaxGenCount,
  priv_cast_fun(GenId,{fun bs_glb_role_mgr:update_role/1,[{GlbRolePojo}]}),
  ?OK.
set_online(RoleId,IsOnline)->
  GenId = RoleId rem ?MaxGenCount,
  priv_cast_fun(GenId,{fun bs_glb_role_mgr:set_online/1,[{RoleId,IsOnline}]}),
  ?OK.

priv_call_fun(GenId,{WorkFun, ParamList}) when is_list(ParamList)->
  Result = gs_glb_role_mgr:call_fun(GenId,{WorkFun, ParamList}),
  Result.
priv_cast_fun(GenId,{WorkFun, ParamList}) when is_list(ParamList)->
  gs_glb_role_mgr:cast_fun(GenId,{WorkFun, ParamList}),
  ?OK.
