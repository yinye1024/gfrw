...
%%%-------------------------------------------------------------------
-module(s2s_lc_offmsg_worker_mgr).
-author("yinye").
...
-export([get_all_msg/2]).

%% ===================================================================================
%% API functions implements
%% ===================================================================================
ets_init()->
  gs_lc_offmsg_worker_mgr:ets_init(),
  ?OK.
start_sup_link()->
  gs_lc_offmsg_worker_mgr:start_sup_link(),
  ?OK.
...
priv_new_child(GenId)when is_integer(GenId)->
  Pid = gs_lc_offmsg_worker_mgr:new_child({GenId}),
  Pid.
...
priv_stop(GenId) when is_integer(GenId)->
  gs_lc_offmsg_worker_mgr:stop(GenId),
  ?OK.

add_msg(RoleId,Msg)->
  priv_cast_fun(RoleId,{fun lc_offmsg_plrudb_mgr:add_msg/1,[{RoleId,Msg}]}),
  ?OK.
remove_by_index(RoleId,MsgIndex)->
  priv_cast_fun(RoleId,{fun lc_offmsg_plrudb_mgr:remove_by_index/1,[{RoleId,MsgIndex}]}),
  ?OK.

get_all_msg(RoleId,LocalCbPojo)->
  priv_cast_fun(RoleId,{fun lc_offmsg_plrudb_mgr:get_all_msg/1,[{RoleId,LocalCbPojo}]}),
  ?OK.

%%priv_call_fun(GenId,{WorkFun, ParamList}) when is_list(ParamList)->
%%  Result = gs_lc_offmsg_worker_mgr:call_fun(GenId,{WorkFun, ParamList}),
%%  Result.
priv_cast_fun(RoleId,{WorkFun, ParamList}) when is_list(ParamList)->
  GenId = RoleId rem ?MaxGenCount,
  gs_lc_offmsg_worker_mgr:cast_fun(GenId,{WorkFun, ParamList}),
  ?OK.
