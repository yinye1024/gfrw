...
init({GenId})->
  ...
  bs_lc_role_worker_mgr:init({GenId}),
  lc_role_worker_pid_mgr:reg(GenId,self()),
...
terminate(Reason,_State=#state{genId = GenId})->
  ...
  ?TRY_CATCH(begin
               bs_lc_role_worker_mgr:terminate(),
               lc_role_worker_pid_mgr:unReg(GenId)
              end),
  ?OK.
...
do_handle_info({persistent},State)->
  ?TRY_CATCH(bs_lc_role_worker_mgr:persistent()),
  erlang:send_after(?GEN_PERSIST_SPAN,self(),{persistent}),
  {?NO_REPLY,State};
do_handle_info({loop_tick},State)->
  ?TRY_CATCH(bs_lc_role_worker_mgr:loop_tick()),
  erlang:send_after(?GEN_TICK_SPAN,self(),{loop_tick}),
  {?NO_REPLY,State};
do_handle_info(Msg,State)->
