...
init({MultiId, MultiInitArgs})->
  ...
  bs_tpl_multi_mgr:init({MultiId, MultiInitArgs}),
  tpl_multi_pid_mgr:reg(MultiId,self()),
...
terminate(Reason,_State=#state{multiId = MultiId})->
  ...
  ?TRY_CATCH(begin
               bs_tpl_multi_mgr:terminate(),
               tpl_multi_pid_mgr:unReg(MultiId)
              end),
  ?OK.
...
do_handle_info({persistent},State)->
  ?TRY_CATCH(bs_tpl_multi_mgr:persistent()),
  erlang:send_after(?GEN_PERSIST_SPAN,self(),{persistent}),
  {?NO_REPLY,State};
do_handle_info({loop_tick},State)->
  ?TRY_CATCH(bs_tpl_multi_mgr:loop_tick()),
  erlang:send_after(?GEN_TICK_SPAN,self(),{loop_tick}),
  {?NO_REPLY,State};
do_handle_info(Msg,State)->
