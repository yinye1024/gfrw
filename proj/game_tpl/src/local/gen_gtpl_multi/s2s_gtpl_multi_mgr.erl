...
%% ===================================================================================
ets_init()->
  gs_gtpl_multi_mgr:ets_init(),
  ?OK.
start_sup_link()->
  gs_gtpl_multi_mgr:start_sup_link(),
  ?OK.

new_child({MultiId, MultiInitArgs})when is_integer(MultiId)->
  Pid = gs_gtpl_multi_mgr:new_child({MultiId, MultiInitArgs}),
  Pid.

stop(MultiId) when is_integer(MultiId)->
  gs_gtpl_multi_mgr:stop(MultiId),
  ?OK.

call_fun(MultiId,{WorkFun,Param}) when is_integer(MultiId)->
  Result = gs_gtpl_multi_mgr:call_fun(MultiId,{WorkFun,Param}),
  Result.
cast_fun(MultiId,{WorkFun,Param}) when is_integer(MultiId)->
  gs_gtpl_multi_mgr:cast_fun(MultiId,{WorkFun,Param}),
  ?OK.
