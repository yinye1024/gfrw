...
init({UserId})->
  ...
  bs_robot_mgr:init({UserId}),
  erlang:send_after(?GEN_TICK_SPAN,self(),{loop_tick}),
  erlang:send_after(?GEN_PERSIST_SPAN,self(),{persistent}),
  {?OK,#state{userId = UserId}}.

terminate(Reason,_State=#state{userId = UserId})->
  ?LOG_INFO({"gen terminate",[reason,Reason,UserId]}),
  ?TRY_CATCH(bs_robot_mgr:terminate(UserId)),
  ?OK.
...
do_handle_info({persistent},State)->
  ?TRY_CATCH(bs_robot_mgr:persistent()),
  erlang:send_after(?GEN_PERSIST_SPAN,self(),{persistent}),
  {?NO_REPLY,State};
do_handle_info({loop_tick},State)->
  ?TRY_CATCH(bs_robot_mgr:loop_1_sec()),
  erlang:send_after(?GEN_TICK_SPAN,self(),{loop_tick}),
  {?NO_REPLY,State};
do_handle_info(Msg,State)->
